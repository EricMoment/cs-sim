...
import ABIMG from '../itemImages/angelic.png'
...
function AngelicBlessing({updateCost}) {
  ...
  const csEffect = () => {
    ...
    if (upgrades > 0) {
      firstEnchant.current ? updateCost((wsCost) => {
        return Math.round((wsCost + 14 + 1.8) * 10) / 10
      }) : updateCost((wsCost) => {
  ...
  return (
    <>
      {passFailMessage(recentResult, 'Angelic Blessing')}
...
export default AngelicBlessing